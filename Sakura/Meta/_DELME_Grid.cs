...
namespace FERHRI.Sakura.Meta
{
    ...
    /// </summary>
    [DataContract]
    public partial class _DELME_Grid
    {
        /// <summary>
        ...
        /// <summary>
        /// Инициализация экземпляра класса типа Enums.Geo.Projection.LATLON.
        /// </summary>
        public _DELME_Grid(int? id, int typeId, int latQ, double latStartMin, double latStepMin, int lonQ, double lonStartMin, double lonStepMin, string Description = null)
        {
            this.Id = id;
            this.TypeId = typeId;
            ...
            switch ((Enums.Geo.Projection)TypeId)
            {
                case Enums.Geo.Projection.LATLON:
                    Test();
                    break;
                default:
                    throw new Exception("(Enums.Geo.Projection)TypeId");
            }
        }

        /// <summary>
        /// Инициализация экземпляра класса типа Enums.Geo.Projection.GAUSS
        /// с предопределёнными широтами.
        /// </summary>
        public _DELME_Grid(int? id, double[] gaussLatsGrd, int lonQ, double lonStartMin, double lonStepMin, string Description = null)
        {
            ...
            this.Id = id;
            this.TypeId = (int)Enums.Geo.Projection.GAUSS;
        ...
        private void Test()
        {
            if (WestMin > _DELME_GeoPoint.GEO_MAXLONGITUDE_GRD * 60 || WestMin < _DELME_GeoPoint.GEO_MINLONGITUDE_GRD * 60
                || EastMin > _DELME_GeoPoint.GEO_MAXLONGITUDE_GRD * 60 || EastMin < _DELME_GeoPoint.GEO_MINLONGITUDE_GRD * 60
                || NorthMin > _DELME_GeoPoint.GEO_MAXLATITUDE_GRD * 60 || NorthMin < _DELME_GeoPoint.GEO_MINLATITUDE_GRD * 60
                || SouthMin > _DELME_GeoPoint.GEO_MAXLATITUDE_GRD * 60 || SouthMin < _DELME_GeoPoint.GEO_MINLATITUDE_GRD * 60
            ) throw new Exception("LATLON grid bounds is wrong : [" + ToString() + "]");

            if (
                (
                    TypeId != (int)Enums.Geo.Projection.GAUSS &&
        ...
        /// </summary>
        public _DELME_GeoRectangle GeoRectangle
        {
            get
            {
                return new _DELME_GeoRectangle(_DELME_GeoPoint.Min2Grd(SouthMin), _DELME_GeoPoint.Min2Grd(NorthMin), _DELME_GeoPoint.Min2Grd(WestMin), _DELME_GeoPoint.Min2Grd(EastMin), "Obtained from Grid");
            }
        }
        //public Grid Truncate(GeoRectangle geoReg)
        //{
        //    if (geoReg.isIntersectGreenwich) throw new Exception("geoReg.isIntersectGreenwich()");
        //    if (TypeId != (int)Enums.Geo.Projection.LATLON) throw new Exception("(gridTypeId != Enums.Geo.Projection.LATLON)");
        ...
        //        case (int)Enums.Geo.Projection.GAUSS:
        //            for (int i = 0; i < LatsMin.Length; i++)
        //            {
        //                if (geoReg.IsLatGrdBelong(LatsMin[i] / 60))
        ...
        /// </summary>
        static public _DELME_Grid getGridMaxOf(_DELME_Grid[] grids)
        {
        ...
        public override string ToString()
        {
            return
                ...
                + "; "
                + (((Enums.Geo.Projection)this.TypeId == Enums.Geo.Projection.GAUSS) ? LatStepMin : LatStepMin / 60) + " : " + LonStepMin / 60
                + "; " +
        ...
        /// <param name="gr"></param>
        /// <returns></returns>
        public List<int> GetNodesIdxs(_DELME_GeoRectangle gr)
        {
            List<int>[] iLatLon = GetLatLonIdxs(gr);
            return GetNodeIdxs(iLatLon);
        }
        public List<_DELME_GeoPoint> GetGeoPoints(_DELME_GeoRectangle gr)
        {
            List<int>[] iLatLon = GetLatLonIdxs(gr);
            List<_DELME_GeoPoint> ret = new List<_DELME_GeoPoint>();
            for (int i = 0; i < iLatLon[0].Count; i++)
            {
                for (int j = 0; j < iLatLon[1].Count; j++)
                {
                    ret.Add(new _DELME_GeoPoint(LatsMin[iLatLon[0][i]] / 60, LonsMin[iLatLon[1][j]] / 60));
                }
            }
            return ret;
        }
        public List<_DELME_GeoPoint> GetGeoPoints()
        {
            List<_DELME_GeoPoint> ret = new List<_DELME_GeoPoint>();
            for (int i = 0; i < LatsMin.Length; i++)
            {
                for (int j = 0; j < LonsMin.Length; j++)
                {
                    ret.Add(new _DELME_GeoPoint(LatsMin[i] / 60, LonsMin[j] / 60));
                }
            }
            return ret;
        }
        ...
        public _DELME_GeoPoint GetNearestPoint(_DELME_GeoPoint geoPoint)
        {
            int[] i = GetNearestPointIdx(geoPoint.LatMin, geoPoint.LonMin);

            if (i != null)
            {
                return new _DELME_GeoPoint(LatsMin[i[0]] / 60, LonsMin[i[1]] / 60);
            }
        ...
        public int[/*iLat, iLon*/] GetNearestPoint(double latMin, double lonMin, _DELME_GeoPoint.NearestType nearestType)
        {
            int[] ret = null;
            ...
            if (nodesIdx != null)
            {
                _DELME_GeoPoint pointIn = new _DELME_GeoPoint(latMin / 60, lonMin / 60);
                int iPoint = 0;
                _DELME_GeoPoint pointSel = new _DELME_GeoPoint(LatsMin[nodesIdx[0][0]] / 60, LonsMin[nodesIdx[0][1]] / 60);
                double distSel = pointSel.getDistMin(pointIn); ;

                for (int i = 1; i < nodesIdx.Count; i++)
                {
                    _DELME_GeoPoint pointCur = new _DELME_GeoPoint(LatsMin[nodesIdx[i][0]] / 60, LonsMin[nodesIdx[i][1]] / 60);
                    double distCur = pointCur.getDistMin(pointIn);

                    switch (nearestType)
                    {
                        case _DELME_GeoPoint.NearestType.Nearest2East:
        ...
        public List<_DELME_GeoPoint> GetNearestPoints(double latMin, double lonMin)
        {
            ...
            if (ll != null)
            {
                ...
                {
                    ret.Add(new _DELME_GeoPoint(LatsMin[ll[i][0]] / 60, LonsMin[ll[i][1]] / 60));
                }
                return ret;
            }
            return null;
        ...
        public _DELME_Grid GetGridByCentralPoint(_DELME_GeoPoint centralPoint, int pointsQ2NorthSouth, int pointsQ2WestEast)
        {
            throw new NotImplementedException();
        ...
        public bool isCompatible(_DELME_Grid grid2)
        {
        ...
        /// <returns>true, если сетки совместимы.</returns>
        static public bool isCompatible(_DELME_Grid grid1, _DELME_Grid grid2)
        {
        ...
        /// <returns>true, если сетки совместимы.</returns>
        static public bool isCompatible(_DELME_Grid grid1, _DELME_Grid grid2, _DELME_GeoRectangle geoRec)
        {
            throw new NotImplementedException();
            ...
            //while (!gs.EOF)
            //{
            //    _DELME_GeoPoint curGeoPoint = new _DELME_GeoPoint(gs.CurLatGrd, gs.CurLonGrd);
        ...
        public List<int>[/*iLat, iLon*/] GetLatLonIdxs(_DELME_GeoRectangle geoRec)
        {
            return new List<int>[] { GetLatIdxs(geoRec), GetLonIdxs(geoRec) };
        }
        public List<int> GetLonIdxs(_DELME_GeoRectangle geoRec)
        {
        ...
        public List<int> GetLatIdxs(_DELME_GeoRectangle geoRec)
        {
            List<int> ret = new List<int>();
        ...
        public List<int[/*iLat,iLon*/]> GetNodeLLIdxs(List<_DELME_GeoPoint> points)
        {
        ...
        public double CurLonMin
        {
            ...
            get
            {
                if (_curLonMin >= _DELME_GeoPoint.GEO_MAXLONGITUDE_MIN)
                    return _curLonMin - _DELME_GeoPoint.GEO_MAXLONGITUDE_MIN;
                if (_curLonMin < 0)
                    return _DELME_GeoPoint.GEO_MAXLONGITUDE_MIN - _curLonMin;
                return _curLonMin;
            }
        }
        /// <summary>
        ...
        public double CurLatGrd
        {
            get
            {
                return _DELME_GeoPoint.Min2Grd(CurLatMin);
            }
        }
        /// <summary>
        ...
        public double CurLonGrd
        {
            get
            {
                return _DELME_GeoPoint.Min2Grd(CurLonMin);
            }
        }
        /// <summary>
        ...
        public bool MoveNext()
        {
            ...
            if (CurPointIdx < PointsQ - 1)
            {
                ...
                if ((CurPointIdx / LonsQ) * LonsQ == CurPointIdx)
                {
                    ...
                    curLatIdx++;
                    if ((Enums.Geo.Projection)TypeId == Enums.Geo.Projection.GAUSS)
                    {
    ...
    ///// <param name="geoPoint">Точка.</param>
    ///// <returns>  List int[latIdx,lonIdx,Idx] </returns>
    //public List<int[/*latIdx,lonIdx,Idx*/]> getNearestGridPoints(GeoPoint _DELME_GeoPoint)
    //{
    ...
    //        default:
    //            throw new Exception("Unknown Geo.Projection =" + (Enums.Geo.Projection)GridTypeId);
    //    }
    //}
    ///// <summary>
    ...
    //    /// Инициализация экземпляра класса.
    //    /// </summary>
    //    public Grid(GeoRectangle reg, int? geoRegId, Enums.Geo.Projection prj, int centerId, char region, char model, double latStepMin, double lonStepMin)
    //    {
    ...
    //        if (prj != Enums.Geo.Projection.LATLON ||
    //            (
    ...
    //        if (GridTypeId != (int)Enums.Geo.Projection.LATLON)
    //            throw new Exception("(GridTypeId != Enums.Geo.Projection.LATLON)");
    ...
    //            + "; "
    //            + (((Enums.Geo.Projection)this.GridTypeId == Enums.Geo.Projection.GAUSS) ? StepYMin : StepYMin / 60) + " : " + StepXMin / 60
    //            + "; " +
    ...
    //        get
    //        {
    //            if (GridTypeId == (int)Enums.Geo.Projection.LATLON) return "LATLON";
    //            if (GridTypeId == (int)Enums.Geo.Projection.GAUSS) return "GAUSS";
    ...
    //    /// <param name="geoPoint">Точка.</param>
    //    /// <returns>true, если принадлежит.</returns>
    //    public bool isPointBelongGridRectangle(GeoPoint _DELME_GeoPoint)
    //    {
    ...
    //    /// <param name="geoPoint">Точка.</param>
    //    /// <returns>Ближайший узел сетки или null, если точка не лежит в пределах региона сетки.</returns>
    //    public _DELME_GeoPoint getNearestGridPoint(GeoPoint _DELME_GeoPoint)
    //    {
    //        if (!isPointBelongGridRectangle(geoPoint))
    //            return null;
    //        _DELME_GeoPoint ret = new _DELME_GeoPoint(Meta.GeoPoint.Min2Grd(LatStartMin), Meta.GeoPoint.Min2Grd(LonStartMin));
    //        double[] dxdy = new double[] { double.MaxValue, double.MaxValue };

    //        GridScaner scan = new GridScaner(this);
    //        do
    //        {
    //            double d = Math.Abs(scan.CurLatMin - _DELME_GeoPoint.LatMin);
    //            if (d < dxdy[0])
    //            {
    //                ret.LatMin = scan.CurLatMin;
    //                dxdy[0] = d;
    //            }
    //            d = Math.Abs(scan.CurLonMin - _DELME_GeoPoint.LonMin);
    //            if (d < dxdy[1])
    //            {
    ...
    //    /// <param name="nearestType">Условие определения близости узла к точке.</param>
    //    /// <returns>[/*iLat, iLon, iPoint*/]</returns>
    //    public int[/*iLat, iLon, iPoint*/] GetNearestPoint(GeoPoint _DELME_GeoPoint, _DELME_GeoPoint.NearestType nearestType)
    //    {
    //        List<int[]> points = getNearestGridPoints(geoPoint);

    //        int iPoint = 0;
    //        _DELME_GeoPoint gridPoint = GetGeoPoint(points[iPoint][0], points[iPoint][1]);
    //        double dist = _DELME_GeoPoint.getDistMin(gridPoint);

    //        for (int i = 0; i < points.Count; i++)
    //        {
    //            _DELME_GeoPoint gridPoint1 = GetGeoPoint(points[i][0], points[i][1]);
    //            double dist1 = _DELME_GeoPoint.getDistMin(gridPoint1);

    //            switch (nearestType)
    //            {
    //                case Meta.GeoPoint.NearestType.Nearest2East:
    //                    if (gridPoint1.LonGrd > _DELME_GeoPoint.LonGrd)
    //                    {
    //                        if (dist1 < dist)
    //                        {
    //                            iPoint = i;
    ...
    //    {
    //        List<object[]> lats = NearestGridLatOrLons(Enums.Geo.LatOrLon.LAT, latGrad * 60);
    //        List<object[]> lons = NearestGridLatOrLons(Enums.Geo.LatOrLon.LON, lonGrad * 60);
    ...
    //                    ret.Add(new object[] 
    //                    {   new _DELME_GeoPoint(((double)lats[i][0]) / 60, ((double)lons[j][0]) / 60), 
    //                        (int)lats[i][1], 
    //                        (int)lons[j][1],
    ...
    //    public List<object[/*(double)latMin,(int)latIdx*/]> NearestGridLatOrLons(Enums.Geo.LatOrLon latOrlon, double minutes)
    //    {

    //        int i, i1, i2;
    //        double[] array = (latOrlon == Enums.Geo.LatOrLon.LAT) ? LatLonMin[0] : LatLonMin[1];
    //        double step = (latOrlon == Enums.Geo.LatOrLon.LAT) ? StepYMin : StepXMin;
    ...
    //        return ret;
    //    }
    //    public _DELME_GeoPoint GetGeoPoint(int iLat, int iLon)
    //    {
    //        return new _DELME_GeoPoint(GetLatMin(iLat) / 60, GetLonMin(iLon) / 60);
    //    }
    ...
    //            default:
    //                throw new Exception("Unknow Geo.Projection=" + (Enums.Geo.Projection)GridTypeId);
    //        }
    //    }
    ...
    //    {
    //        _DELME_GeoPoint cPoint = getNearestGridPoint(centralPoint);
    //        if (cPoint == null) return null;
    ...
    //    public _DELME_GeoPoint _DELME_GeoPoint(int latIdx, int lonIdx)
    //    {
    //        return new _DELME_GeoPoint(Meta.GeoPoint.Min2Grd(LatStartMin + latIdx * StepYMin), Meta.GeoPoint.Min2Grd(LonStartMin + lonIdx * StepXMin));
    //    }
    //    /// <summary>
    ...
    //                curLatIdx++;
    //                if ((Enums.Geo.Projection)GridTypeId == Enums.Geo.Projection.GAUSS)
    //                {
