...
namespace SOV.DbNpgsql
{
    ...
    public class ADbNpgsql
    {
        ...
        static public string ConnectionStringUpdateDataBase(string connectionString, string newDataBaseName)
        {
            string ret = "";
            foreach (var item in connectionString.Split(';'))
            {
                string[] pair = item.Split('=');
                if(pair[0].Trim().ToUpper() == "DATABASE")
                {
                    pair[1] = newDataBaseName;
                }
                ret += pair[0] + "=" + pair[1];
            }
            return ret;
        ...
        static string ConnectionStringUpdateAttr(string connectionString,string attrName,  string newAttrValue)
        {
            string ret = "";
            foreach (var item in connectionString.Split(';'))
            {
                string[] pair = item.Split('=');
                if (pair[0].Trim().ToUpper() == attrName.Trim().ToUpper())
                {
                    pair[1] = newAttrValue;
                }
                ret += pair[0] + "=" + pair[1];
